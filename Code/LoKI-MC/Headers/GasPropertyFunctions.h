...
namespace GasPropertyFunctions{
	...
	/*
		'funcionPointer' is defined in Gas.h in the following way:
	...
	*/
	...
	double nitrogenHeatCapacity(GasType* gas, std::vector<double> &argumentArray, std::vector<std::string> &argumentStrArray, WorkingConditions* workCond){
		// heat capacity for nitrogen [J K-1 mol-1]
		double heatCapacity;
		if (argumentArray[0] == 1){ // constant pressure
			heatCapacity = 29.1 + 2494.2/(553.4*std::sqrt(M_PI/2.0))*std::exp(-2.0*std::pow((workCond->gasTemperature-1047.4)/553.4, 2));
		}
		else{// constant volume
			heatCapacity = -Constant::avogadro*Constant::boltzmann + 29.1 + 2494.2/(553.4*std::sqrt(M_PI/2.0))*std::exp(-2.0*std::pow((workCond->gasTemperature-1047.4)/553.4, 2));
		}
	...
	double oxygenHeatCapacity(GasType* gas, std::vector<double> &argumentArray, std::vector<std::string> &argumentStrArray, WorkingConditions* workCond){
		// heat capacity for oxygen [J K-1 mol-1]
		double heatCapacity;
		if (argumentArray[0] == 1){ // constant pressure
			heatCapacity = 28.8 + 6456.2/(788.3*std::sqrt(M_PI/2.0))*std::exp(-2.0*std::pow((workCond->gasTemperature-1006.9)/788.3, 2));
		}
		else{ // constant volume
			heatCapacity = -Constant::avogadro*Constant::boltzmann + 28.8 + 6456.2/(788.3*std::sqrt(M_PI/2.0))*std::exp(-2.0*std::pow((workCond->gasTemperature-1006.9)/788.3, 2));
		}
		// change energy units to eV
		heatCapacity /= Constant::electronCharge;
		// store value on gas object properties
	...
	void evaluateGasPropertyFunction(std::string propertyString, GasType* gas, std::string property, std::vector<double> argumentArray, std::vector<std::string> argumentStrArray, WorkingConditions* workCond){
		...
		if (propertyFuncStr == "nitrogenHeatCapacity"){
			...
			if (property != "heatCapacity"){
				...
				Message::error("Wrong number of arguments when evaluating nitrogenHeatCapacity function. It must be only one and either '1' or '2', for capacities at constant pressure or constant volume, respectively.\nCheck input file");
			}
			else if (argumentArray[0] != 1 && argumentArray[0] != 2){
				Message::error("Wrong argument when evaluating nitrogenHeatCapacity function. It must be either '1' or '2', for capacities at constant pressure or constant volume, respectively \nCheck input file");
			}
			...
			if (property != "heatCapacity"){
				...
				Message::error("Wrong number of arguments when evaluating oxygenHeatCapacity function. It must be only one and either '1' or '2', for capacities at constant pressure or constant volume, respectively.\nCheck input file");
			}
			else if (argumentArray[0] != 1 && argumentArray[0] != 2){
				Message::error("Wrong argument when evaluating oxygenHeatCapacity function. It must be either '1' or '2', for capacities at constant pressure or constant volume, respectively.\nCheck input file");
			}		
			// calculate the values of the property
		...
		// ----- property functions related with thermal conductivity ----- //
			...
			Message::error(std::string("Error! Trying to use the property function '") + propertyFuncStr + "' which is not defined in the code.\n" +
						"Add it at 'GasPropertyFunctions.h' and 'GasPropertyFunctions.C', and do not forget to put it also at the function 'evaluateGasPropertyFunction'.");
		}
