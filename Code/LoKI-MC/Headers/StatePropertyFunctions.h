...
namespace StatePropertyFunctions{
	...
	/*
      The 'functionPointer' is defined in State.h in the following way:
	...
	*/
	...
	template <class StateType>
	void rotationalDegeneracy_NO(std::vector<StateType*> &stateArray, std::vector<double> &argumentArray, std::vector<std::string> &argumentStrArray, WorkingConditions* workCond){
		double J;
		for (auto& state: stateArray){
			J = std::stod(state->rotLevel);
			state->statisticalWeight = 3.0*(2.0*J + 1.0);
		}
	}
	//****************************************************//
	...
	template <class StateType>
	void rigidRotorEnergy_NO(std::vector<StateType*> &stateArray, std::vector<double> &argumentArray, std::vector<std::string> &argumentStrArray, WorkingConditions* workCond){
		double rotConst, omega;
		if (stateArray[0]->eleLevel == "X_1/2"){
			rotConst = 0.2073e-3;
			omega = 0.5;
		}
		else{
		...
		for(auto& state: stateArray){
			double J = std::stod(state->rotLevel);
			state->energy = rotConst*(J*(J + 1.0) - omega*omega);
		}
	}
	...
	void evaluateStatePropertyFunction(std::string propertyString, std::vector<StateType*> &stateArray, std::string property, std::vector<double> argumentArray, std::vector<std::string> argumentStrArray, WorkingConditions* workCond){
		...
		else if (propertyFuncStr == "rotationalDegeneracy_NO"){
			// check if the parameters are appropriate to this function
			if (property != "statisticalWeight"){
				Message::error(std::string("Trying to use rotationalDegeneracy_NO function to set up property ") + property + ". \nCheck input file");
			}
			else if (!argumentArray.empty()){
				Message::error("Wrong number of arguments when evaluating rotationalDegeneracy_NO function. \nCheck input file");
			}
			else if (stateArray[0]->type != "rot"){
				Message::error(std::string("Trying to assign rotationalDegeneracy_NO to non rotational state ") + stateArray[0]->name + ". \nCheck input file");
			}
			// calculate the values of the property
			rotationalDegeneracy_NO(stateArray, argumentArray, argumentStrArray, workCond);
			// assign this function to each state (to be used later on, if necessary)
			for (auto& state: stateArray){
				state->statisticalWeightFunc = rotationalDegeneracy_NO;
				state->statisticalWeightParams = argumentArray;
				state->statisticalWeightStrParams = argumentStrArray;
			}
		}	
		...
		else if (propertyFuncStr == "rigidRotorEnergy_NO"){
			// check if the parameters are appropriate to this function
			StateType* firstState = stateArray[0];
			if (property != "energy"){
				Message::error(std::string("Trying to use rigidRotorEnergy_NO function to set up property ") + property + ". \nCheck input file");
			}
			else if (!argumentArray.empty()){
				Message::error("Wrong number of arguments when evaluating rigidRotorEnergy_NO function. \nCheck input file");
			}
			else if (firstState->type != "rot"){
				Message::error(std::string("Trying to assign rigidRotorEnergy_NO to non rotational state ") + firstState->name + "\nCheck input file.");
			}
			// calculate the values of the property
			rigidRotorEnergy_NO(stateArray, argumentArray, argumentStrArray, workCond);
			// assign this function to each state (to be used later on, if necessary)
			for (auto& state: stateArray){
				state->energyFunc = rigidRotorEnergy_NO;
				state->energyParams = argumentArray;
				state->energyStrParams = argumentStrArray;
			}	
		}	
		...
		// ----- property functions related with state populations ----- //
			...
			Message::error(std::string("Error! Trying to use the property function '") + propertyFuncStr + "' which is not defined in the code.\n" +
						"Add it at 'StatePropertyFunctions.h' and 'StatePropertyFunctions.C', and do not forget to put it also at the function 'evaluateStatePropertyFunction'.");
		}
